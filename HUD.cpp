...
HUD::HUD(SpriteManager* spritemanager)
{
	...
	m_blueArrow =  m_spritemanager->GetSprite("blue_arrow.png", 62, 63);
	m_redArrow =  m_spritemanager->GetSprite("red_arrow.png", 62, 63);

	m_font.loadFromFile("../Sprites/Minnie.ttf");
	...
	m_scoretext->setPosition(1700, 960);
	...
	m_happypilltext->setPosition(m_powerupframePOS + sf::Vector2f(10, 145));
	m_happypilltext->setCharacterSize(30);
	m_happypilltext->setFont(m_font);
	m_happypilltext->setColor(sf::Color::Red);
	m_happypilltext->setStyle(sf::Text::Regular);

	m_bluecowtext = new sf::Text;
	m_bluecowtext->setPosition(m_powerupframePOS + sf::Vector2f(235, 145));
	m_bluecowtext->setCharacterSize(30);
	...
	m_redArrowTimer = 25.0f;
	m_blueArrowTimer = 5.0f;
}
...
void HUD::Update(Player* player, float &deltatime)
{
	m_blueArrowTimer -= deltatime;
	m_redArrowTimer -= deltatime;
	...
	m_redArrow->setPosition(m_indicator->getPosition() + sf::Vector2f(20, -50));
	m_blueArrow->setPosition(m_indicator->getPosition() + sf::Vector2f(20, -50));
...
float HUD::GetBlueArrowTimer()
{
	return m_blueArrowTimer;
}

sf::Sprite* HUD::GetBlueArrow()
{
	return m_blueArrow;
}

float HUD::GetRedArrowTimer()
{
	return m_redArrowTimer;
}

sf::Sprite* HUD::GetRedArrow()
{
	return m_redArrow;
}